...
export function ChatInput({ onSend, disabled = false, placeholder = "메시지 입력…" }: Props) {
  ...
  const [slashQuery, setSlashQuery] = useState<string | null>(null); // null = 메뉴 닫힘
  const [activeIndex, setActiveIndex] = useState(0);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  // 슬래시 메뉴 필터 목록 (activeIndex 범위 계산용)
  const filteredCount = slashQuery !== null
  ...
  // value가 외부에서 리셋될 경우 메뉴도 닫기
  useEffect(() => {
    if (value === "") setSlashQuery(null);
  }, [value]);
