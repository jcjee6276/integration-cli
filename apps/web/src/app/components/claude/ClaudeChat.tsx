...
export function ClaudeChat() {
  const { messages, status, sessionId, sendMessage, createSession } = useClaudeSession();
  const bottomRef = useRef<HTMLDivElement>(null);

  const isInputDisabled =
    status === 'disconnected' || status === 'connecting' || !sessionId;
  ...
  return (
    <div className="flex h-screen flex-col bg-white dark:bg-zinc-950">
      ...
      {sessionId && (
        <InputArea onSend={sendMessage} disabled={isInputDisabled || isThinking} />
      )}
    </div>
  );
}
