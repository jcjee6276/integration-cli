...
function ComingSoonCard({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-gray-900/[0.07] bg-gray-900/[0.02] p-4 opacity-50 dark:border-white/[0.07] dark:bg-white/[0.02]">
      <span
        className={`flex h-9 w-9 items-center justify-center rounded-lg border border-gray-900/[0.07] dark:border-white/[0.07] ${iconBg}`}
      >
        {icon}
      </span>
      <div className="flex flex-1 flex-col">
...
export const AgentStatusModal = memo(function AgentStatusModal({ open, onClose }: AgentStatusModalProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        claudeStatus: claude.status === "fulfilled" ? claude.value : null,
        geminiStatus: gemini.status === "fulfilled" ? gemini.value : null,
        loading: false,
      });
    } catch {
      if (requestIdRef.current !== requestId) return;
...
export const AgentStatusModal = memo(function AgentStatusModal({
  open,
  onClose,
}: AgentStatusModalProps) {
  const { claudeStatus, geminiStatus, loading, refresh } = useAgentStatusModal(open);
