...
export function useTaskList(open: boolean) {
  ...
  const applyFetchedTasks = useCallback(async (isCanceled: () => boolean = () => false) => {
    try {
  ...
  useEffect(() => {
    ...
    let canceled = false;
    queueMicrotask(() => {
      void applyFetchedTasks(() => canceled);
    });
    return () => {
      canceled = true;
    };
  }, [open, applyFetchedTasks]);
  ...
  const execute = useCallback(
    async (id: string, agentTestCodePreferences: AgentTestCodePreference[] = []) => {
      setActioningId(id);
      try {
  ...
  const rerun = useCallback(
    async (id: string, supplementNote?: string, writeTestCode?: boolean) => {
      setActioningId(id);
      try {
  ...
  const rerunAgent = useCallback(
    async (id: string, agentId: number, supplementNote?: string, writeTestCode?: boolean) => {
      setActioningId(id);
      try {
  ...
  const onEditDone = useCallback((updated: Task) => {
