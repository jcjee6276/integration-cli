...
/** rehype-highlight가 만든 토큰 span 트리에서 원본 텍스트를 재귀적으로 추출 */
  ...
  return (
    <div className="group relative my-3">
      <button
        type="button"
        onClick={() => void copy()}
        className="absolute top-2 right-2 z-10 cursor-pointer rounded-md border border-gray-900/[0.1] bg-white/85 px-2 py-1 text-[11px] font-medium text-gray-900/55 opacity-0 backdrop-blur transition-opacity group-hover:opacity-100 hover:text-gray-900/80 dark:border-white/[0.14] dark:bg-white/[0.1] dark:text-white/55 dark:hover:text-white/85"
      >
        {copied ? "복사됨" : "복사"}
      </button>
      <pre className="overflow-x-auto rounded-lg border border-gray-900/[0.06] bg-gray-900/[0.03] p-4 text-xs leading-relaxed dark:border-white/[0.06] dark:bg-white/[0.03]">
        {children}
      </pre>
    </div>
  );
    ...
    <div className="min-w-0 [overflow-wrap:anywhere] break-words">
      <ReactMarkdown
        ...
        components={{
          ...
          hr() {
            return <hr className="my-4 border-gray-900/[0.08] dark:border-white/[0.08]" />;
          },
          table({ children }) {
            return (
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-lg border border-gray-900/[0.07] text-xs dark:border-white/[0.07]">
      <div className="flex items-center gap-1.5 border-b border-gray-900/[0.07] bg-gray-900/[0.03] px-3 py-1.5 dark:border-white/[0.07] dark:bg-white/[0.03]">
        ...
        <span className="font-mono font-medium text-gray-900/55 dark:text-white/55">
          {toolUse.tool}
        </span>
      </div>
      <pre className="overflow-x-auto bg-gray-900/[0.02] px-3 py-2 font-mono break-all whitespace-pre-wrap text-emerald-700 dark:bg-white/[0.02] dark:text-emerald-400/80">
        {preview}
      </pre>
    </div>
  );
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-amber-500/30 bg-amber-500/[0.08] px-2 py-0.5 text-[10px] font-medium text-amber-600 dark:text-amber-400">
      <svg viewBox="0 0 16 16" fill="currentColor" className="h-2.5 w-2.5 shrink-0">
        <path
          fillRule="evenodd"
          d="M6.457 1.047c.659-1.234 2.427-1.234 3.086 0l6.082 11.378A1.75 1.75 0 0114.082 15H1.918a1.75 1.75 0 01-1.543-2.575L6.457 1.047zM9 11a1 1 0 11-2 0 1 1 0 012 0zm-.25-5.25a.75.75 0 00-1.5 0v2.5a.75.75 0 001.5 0v-2.5z"
          clipRule="evenodd"
        />
      </svg>
      한도 초과
...
function AgentAvatar({ agentId }: { agentId?: AgentId }) {
  ...
  return (
    <div
      className={`flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full ${cfg.bg}`}
    >
      {cfg.icon}
    </div>
  );
  ...
  if (isUser) {
    return (
      <div className="flex w-full justify-end gap-3">
        ...
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-600/80 text-xs font-bold text-white">
          U
        </div>
      </div>
    );
      ...
      <div className="max-w-[75%] min-w-0 rounded-2xl rounded-tl-sm border border-gray-900/[0.06] bg-gray-900/[0.04] px-4 py-3 text-sm text-gray-900/80 dark:border-white/[0.06] dark:bg-white/[0.04] dark:text-white/80">
        {content ? (
          ...
          <span className="flex gap-1 py-1">
            ...
            <span className="animate-bounce text-gray-900/40 [animation-delay:0.15s] dark:text-white/40">
              ●
            </span>
            <span className="animate-bounce text-gray-900/40 [animation-delay:0.3s] dark:text-white/40">
              ●
            </span>
          </span>
        )}
  ...
  return (
    <div className="flex w-full justify-center">
      <div className="w-full max-w-[90%] overflow-hidden rounded-xl border border-gray-900/[0.07] bg-gray-900/[0.02] text-xs dark:border-white/[0.07] dark:bg-white/[0.02]">
        ...
        <div className="px-4 py-3 font-mono whitespace-pre-wrap text-gray-900/55 dark:text-white/55">
          {content}
        </div>
      </div>
    </div>
