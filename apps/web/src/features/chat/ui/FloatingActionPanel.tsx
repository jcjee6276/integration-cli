...
import { fetchArchivedTasks, unarchiveTask, deleteTask } from "@/features/tasks/api/tasks.api";
...
const STATUS_COLOR: Record<string, string> = {
  ...
  completed: "text-emerald-600 bg-emerald-500/[0.08] dark:text-emerald-400 dark:bg-emerald-400/[0.12]",
...
function ArchiveTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setTasks(await fetchArchivedTasks());
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
        <svg viewBox="0 0 16 16" fill="currentColor" className="h-6 w-6 text-gray-900/15 dark:text-white/15">
  ...
  return (
    <ul className="flex flex-col divide-y divide-gray-900/[0.05] dark:divide-white/[0.05]">
      {tasks.map((task) => {
        const acting = actioningId === task.id;
        return (
          <li key={task.id} className="flex items-start gap-3 px-4 py-3">
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-medium text-gray-900/70 dark:text-white/70">{task.title}</p>
              <div className="mt-1 flex items-center gap-2">
                <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${STATUS_COLOR[task.status] ?? STATUS_COLOR.pending}`}>
            ...
            <div className="flex shrink-0 items-center gap-1">
              <button
                type="button"
                disabled={acting}
                onClick={() => void handleUnarchive(task.id)}
                title="보관 해제"
                className="cursor-pointer rounded-md p-1 text-gray-900/30 transition-colors hover:bg-gray-900/[0.05] hover:text-gray-900/70 disabled:cursor-not-allowed disabled:opacity-40 dark:text-white/30 dark:hover:bg-white/[0.05] dark:hover:text-white/70"
              >
              ...
              <button
                ...
                title="삭제"
                className="cursor-pointer rounded-md p-1 text-gray-900/25 transition-colors hover:bg-red-500/[0.06] hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-40 dark:text-white/25 dark:hover:bg-red-400/[0.08] dark:hover:text-red-400"
              >
                <svg viewBox="0 0 16 16" fill="currentColor" className="h-3.5 w-3.5">
