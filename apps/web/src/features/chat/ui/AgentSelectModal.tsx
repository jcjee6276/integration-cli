...
const AgentCard = memo(function AgentCard({ agent, connectionStatus, onSelect }: AgentCardProps) {
...
export const AgentSelectModal = memo(function AgentSelectModal({ open, onClose, onSelect, connectionStatusByAgent }: AgentSelectModalProps) {
  ...
  return (
    <AgentSelectModalView onClose={onClose} onSelect={onSelect} statusByAgent={statusByAgent} />
  );
}

function AgentSelectModalView({ onClose, onSelect, statusByAgent }: AgentSelectModalViewProps) {
  ...
  return (
    <Modal open onClose={onClose} title="에이전트 선택" maxWidth="max-w-md" zIndex="z-[60]">
      <div className="flex flex-col gap-2.5">
        {AGENTS.map((agent) => (
          <AgentCard
            key={agent.id}
            agent={agent}
            connectionStatus={statusByAgent[agent.id] ?? "disconnected"}
...
function AgentSelectModalComponent({
  ...
  if (connectionStatusByAgent) {
    return (
      <AgentSelectModalView
        onClose={onClose}
        onSelect={onSelect}
        statusByAgent={connectionStatusByAgent}
      />
    );
  }

  return <AgentSelectModalWithStore onClose={onClose} onSelect={onSelect} />;
}

export const AgentSelectModal = memo(AgentSelectModalComponent, (prev, next) =>
  !prev.open && !next.open
    ? true
    : prev.open === next.open &&
      prev.onClose === next.onClose &&
      prev.onSelect === next.onSelect &&
      prev.connectionStatusByAgent === next.connectionStatusByAgent,
);
