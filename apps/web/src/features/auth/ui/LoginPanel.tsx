  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-6">
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/[0.08] bg-orange-500/[0.08]">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-8 w-8 text-orange-400/80">
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
          </svg>
        </div>
        <h2 className="text-xl font-semibold text-white/90">Claude Code 로그인 필요</h2>
        ...
        <div className="flex w-full max-w-lg flex-col gap-4">
          {loginUrls.length > 0 && (
            <div className="flex flex-col gap-2">
              <p className="text-xs font-medium text-white/35">브라우저에서 아래 링크를 열어 인증을 완료하세요:</p>
              {loginUrls.map((url) => (
                <a
                  key={url}
                  href={url}
          ...
          {loginOutput && (
            <pre className="max-h-48 overflow-y-auto rounded-lg border border-white/[0.07] bg-white/[0.02] px-4 py-3 font-mono text-xs leading-relaxed whitespace-pre-wrap text-white/45">
              {loginOutput}
            </pre>
          )}
          ...
          {loginUrls.length === 0 && (
            <div className="flex items-center justify-center gap-2 text-sm text-white/30">
              <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/[0.08] border-t-orange-500" />
              로그인 프로세스를 시작하는 중…
            </div>
          )}

          <button onClick={onCancel} className="text-xs text-white/25 transition-colors hover:text-white/50">
            취소
          </button>
        </div>
      )}
      ...
      {isDone && (
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm font-medium text-emerald-400">로그인 완료</p>
          <p className="text-xs text-white/25">잠시 후 자동으로 이동합니다…</p>
        </div>
      )}
      ...
      {isError && (
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          {loginOutput && (
            <pre className="max-h-32 w-full max-w-lg overflow-y-auto rounded-lg border border-white/[0.07] bg-white/[0.02] px-4 py-3 font-mono text-xs whitespace-pre-wrap text-white/40">
              {loginOutput}
            </pre>
          )}
          <button
